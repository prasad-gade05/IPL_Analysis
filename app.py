...
def load_headline_metrics(s_start: int, s_end: int) -> dict:
    """Load aggregate metrics for the headline cards."""
    df = query(
        """
        SELECT
...
def load_season_evolution(s_start: int, s_end: int):
    """Matches per season (bar) and teams per season (line)."""
    return query(
        """
...
def load_champions(s_start: int, s_end: int):
    """Number of titles per team within the selected range."""
    return query(
        """
...
def load_latest_season_info(s_end: int):
    """Highlights for the latest season in the selected range."""
    return query(
        """
...
def load_top_batters_season(season: int, limit: int = 5):
    """Top run-scorers in a given season."""
    return query(
        """
...
def load_top_bowlers_season(season: int, limit: int = 5):
    """Top wicket-takers in a given season."""
    return query(
        """
...
def load_alltime_top_batters(s_start: int, s_end: int, limit: int = 10):
    """All-time top run-scorers within the selected range."""
    return query(
        """
...
def load_alltime_top_bowlers(s_start: int, s_end: int, limit: int = 10):
    """All-time top wicket-takers within the selected range."""
    return query(
        """
        SELECT bowler, SUM(wickets) AS total_wickets,
               SUM(runs_conceded) AS runs_conceded,
...
def load_latest_top_scorer(season: int):
    """Top run-scorer name and runs for the latest season."""
    df = query(
        """
...
def load_latest_top_wicket_taker(season: int):
    """Top wicket-taker name and wickets for the latest season."""
    df = query(
        """
        SELECT bowler, SUM(wickets) AS total_wickets
        FROM player_bowling
...
def build_evolution_chart(df):
    """Dual-axis bar + line chart: matches (bars) and teams (line)."""
    fig = go.Figure()
    ...
    fig.add_trace(
        go.Bar(
            ...
            y=df["total_matches"],
            name="Matches",
            marker_color=IPL_COLORWAY[0],
            opacity=0.85,
            text=df["total_matches"],
...
def build_champions_chart(df):
    """Horizontal bar chart of IPL titles per team."""
...
def build_horizontal_bar(df, x_col, y_col, title, color=None, height=350):
    """Generic styled horizontal bar helper."""
...
def main():
    ...
    s_start, s_end = st.session_state["season_range"]
    ...
    # ── 3. IPL Evolution Timeline ────────────────────────────────────────────
    evo_df = load_season_evolution(s_start, s_end)
    if not evo_df.empty:
        st.plotly_chart(build_evolution_chart(evo_df), use_container_width=True)

    st.divider()

    # ── 4. Champion Roll of Honor ────────────────────────────────────────────
    champs_df = load_champions(s_start, s_end)
    if not champs_df.empty:
        st.plotly_chart(build_champions_chart(champs_df), use_container_width=True)

    st.divider()

    # ── 5. Latest Season Highlights ──────────────────────────────────────────
    ...
    # ── 6 & 7. Latest Season Top 5 Batters & Bowlers ─────────────────────────
    ...
    with col_bat:
        if not top_bat.empty:
            fig = build_horizontal_bar(
                ...
                x_col="total_runs",
                y_col="batter",
                title=f"🏏 Top 5 Run-Scorers — {s_end}",
                color=IPL_COLORWAY[3],
            )
    ...
    with col_bowl:
        if not top_bowl.empty:
            fig = build_horizontal_bar(
                ...
                x_col="total_wickets",
                y_col="bowler",
                title=f"🎯 Top 5 Wicket-Takers — {s_end}",
                color=IPL_COLORWAY[2],
            )
            st.plotly_chart(fig, use_container_width=True)
        else:
    ...
    # ── 8 & 9. All-Time Top 10 Batters & Bowlers ─────────────────────────────
    ...
    with col_at_bat:
        if not at_bat.empty:
            fig = build_horizontal_bar(
                ...
                x_col="total_runs",
                y_col="batter",
                title="🏏 All-Time Top 10 Run-Scorers",
                color=IPL_COLORWAY[0],
                height=420,
            )
            st.plotly_chart(fig, use_container_width=True)
    ...
    with col_at_bowl:
        if not at_bowl.empty:
            fig = build_horizontal_bar(
                ...
                x_col="total_wickets",
                y_col="bowler",
                title="🎯 All-Time Top 10 Wicket-Takers",
                color=IPL_COLORWAY[1],
                height=420,
            )
            st.plotly_chart(fig, use_container_width=True)
    ...
    # ── 10. Feature Navigation Tiles ─────────────────────────────────────────
    st.subheader("🗺️ Explore the Platform")

    page_tiles = [
        ("📅", "Season Hub", "Complete story of any IPL season — points table, race charts, match results"),
        ("🏆", "Leaderboards", "All-time batting, bowling & team rankings with filterable tables"),
        ("🏏", "Player Profile", "Deep-dive into any player's career stats, form & matchups"),
        ("👥", "Team Profile", "Franchise analytics — win rates, squads, head-to-head records"),
        ("🏟️", "Venue Intelligence", "Ground-specific scoring patterns, toss impact & pace vs spin"),
        ("⚔️", "Head-to-Head", "Batter vs Bowler and Team vs Team battles"),
        ("📊", "Phase Analysis", "Powerplay, middle & death overs — who dominates where?"),
        ("🔥", "Pressure & Momentum", "Dot-ball cascades, chase dynamics & clutch performances"),
        ("📈", "Trends & Evolution", "How the IPL has changed across 18 seasons"),
        ("🎯", "Records & Anomalies", "Every IPL record, milestone & statistical oddity"),
        ("📋", "Match Center", "Ball-by-ball replay & interactive scorecards"),
        ("🏆", "Tournament Structure", "Playoff scenarios, qualification math & format changes"),
        ("🔍", "Explorer", "Custom SQL-style query builder on the full dataset"),
    ]

    # 4 columns × rows
    row_size = 4
    for row_start in range(0, len(page_tiles), row_size):
        cols = st.columns(row_size)
        for idx, (icon, title, desc) in enumerate(page_tiles[row_start : row_start + row_size]):
            with cols[idx]:
                st.markdown(
                    f"""
                    <div style="
                    ...
                    ">
                        ...
                        <p style="font-size:0.82rem; color:#AAAAAA; margin:0;">
                            {desc}
                        </p>
                    </div>
                    """,
    ...
    st.markdown(
        ...
        "Built with ❤️ using Streamlit • DuckDB • Plotly &nbsp;|&nbsp; "
        "Data: IPL Ball-by-Ball 2008–2025"
        "</p>",
        unsafe_allow_html=True,
    )
